...
<div style="text-align: justify;">
  Tables below provide a general overview of the data package. Headings redirect to the associated YODA landing page. Direct downloads are possible via the download button.
</div>
...
<div style="text-align:left;">
  <h3>
    <a href="https://public.yoda.uu.nl/geo/UU01/16EJ3Y.html" target="_blank" rel="noopener">
      Quality Assurance
    </a>
  </h3>
</div>

Regions where GLOBGM has been shown to provide less accurate results due to practical and theoretical limitations; namely, karst aquifers, permafrost and mountainous regions each of which is contained by a seperate variable (static_quality_flags.nc). In addition, regions where spin up issues are prevalent are also indicated - values are explained the by the matrix below (spin_up_*.nc). 

![Regions where GLOBGM has been shown to provide less accurate results due to practical and theoretical limitations; namely, karst aquifers, permafrost and mountainous regions. In addition, regions where spin up issues are prevalent are also indicated.](../images/QA_figure.png){fig-align="center"}
...
df = pd.read_csv('quality_assurance.csv', header=None, names=['path', 'size', 'hash'])
df[['Name']] = df['path'].str.split('/', n=1, expand=True)
# Handle both GiB and MiB, converting MiB to GiB (divide by 1000)
def parse_size(size_str):
  if 'GiB' in size_str:
    return float(size_str.replace(' GiB', ''))
  elif 'MiB' in size_str:
    return round(float(size_str.replace(' MiB', '')) / 1000, 3)
  else:
    return None

df['Size'] = df['size'].apply(parse_size)
df['Download'] = 'https://geo.public.data.uu.nl/vault-globgm-cmip6-quality/research-globgm-cmip6-quality[1755500055]/original/' + df['path']
df = df[['Name', 'Size', 'Download']]
df.columns = ["Name", "Size (Gb)", "Download"]
